...
- **In-app hero draft** (was Planned #3 — done). For `heroDraft` cubes, heroes are drafted **after** the 4 card packs (committing to heroes before seeing cards was too hard) and the phase is **turn-based** (the only way to give a 2-player table 3 heroes without ever revisiting a booster). `generateHeroDraftPacks(heroes, players)` builds N shared boosters sized to the table (`players` heroes each), `N = min(3, floor(12/players))` = heroes per player → **3 at 2-4 players, 2 at 5-6**. Each booster is drafted one pick per player in snake order (`heroOrderFor`), so nobody picks twice from the same booster. `applyPick` switches `drafting → heroDraft` when the last card pack empties; `applyHeroPick` runs the snake and ends at `done`. State: `heroBoosters`/`heroBoosterIndex`/`heroCurrent`/`heroOrder`/`heroTurnPos`/`heroPicks`. Drafted heroes merge into each seat's Results pool (deck still uses ≤1 hero). `Draft.jsx` is phase-aware (shared booster, "Booster X/N", off-turn "waiting for <name>"); the old manual `HeroDraftInfo.jsx` panel was deleted. Per-cube `maxPlayers` (default 4). Non-cube modes unaffected (hero path only activates when `cube.heroDraft` is set).
