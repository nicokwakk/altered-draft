...
- **In-app hero draft** (was Planned #3 — done). For `heroDraft` cubes, heroes are drafted in turn-based segments **interleaved between card rounds** per `cube.heroRules.schedule` (LuigiNico: 1 hero after round 1, the rest after round 2; future cubes can schedule differently; default = all after the final round). Turn-based is the only way to give a 2-player table 3 heroes without revisiting a booster. `generateHeroDraftPacks(heroes, players)` builds N shared boosters sized to the table (`players` each), `N = min(3, floor(12/players))` → **3 at 2-4 players, 2 at 5-6**; each is drafted one pick per player in snake order (`heroOrderFor`). `resolveHeroStops` turns the schedule into cumulative stops; `applyPick` pauses into `heroDraft` at each stop (leaving round/remainingPacks intact) and `applyHeroPick` resumes the card draft after the segment (or `done` after the final round). State: `heroBoosters`/`heroStops`/`heroDrafted`/`heroBoosterIndex`/`heroCurrent`/`heroOrder`/`heroTurnPos`/`heroSegmentThrough`/`heroPicks`. Drafted heroes merge into each seat's Results pool (deck still uses ≤1 hero). `Draft.jsx` is phase-aware; the old manual `HeroDraftInfo.jsx` panel was deleted. Per-cube `maxPlayers` (default 4). Non-cube modes unaffected (hero path only activates when `cube.heroDraft` is set).
