...
**In-app hero draft** (`cube.heroDraft` + `cube.heroes` = 12 refs): heroes are NOT in the card packs — they're drafted in a dedicated `heroDraft` phase that runs BEFORE the card draft. `generateHeroDraftPacks(heroes, players)` partitions the 12 heroes into ONE equal booster per seat (disjoint, no dups); if 12 isn't divisible by the player count the remainder is dropped at random (shuffle-first), so every booster is the same size — e.g. 5 players → drop 2 → five boosters of 2; 4 → four of 3; 2 → two of 6. Each player ends with `floor(12/players)` heroes. The boosters rotate and are picked exactly like card packs (`applyHeroPick` mirrors `applyPick` on parallel `state.heroPacks`/`heroPicks`/`heroWaitingFor`/`heroRound` fields, same optimistic-concurrency `version` write). `buildInitialState(config, players, cardPacks, heroPacks)` seeds the (dormant) round-1 card packs AND the hero boosters, opening in phase `heroDraft`; when the hero boosters empty, `applyHeroPick` flips phase to `drafting` and arms the card draft. Drafted heroes are merged into each seat's pool on the Results page (`heroPicks` ++ `picks`, heroes first) so they appear in All Picks / deck / export — the player still puts only 1 hero in the actual deck (validity = ≤1 hero). `Draft.jsx` is phase-aware: the same `CardGrid`/`doPick`/timer drive the hero pack, with a "Hero Draft — Pick X/Y" header and a "Your heroes" strip during the card draft. Lobby + Draft navigate to `/draft` for BOTH `heroDraft` and `drafting` phases. (Replaces the old manual `HeroDraftInfo.jsx` panel, now deleted.)
