...
**Per-copy faction tokens (out-of-faction cards):** a multi-copy cube relabels cards to faction columns via the parallel `cube.factions` array, and the SAME ref can sit in two columns at once (intentional OOF tech, e.g. Foundry Armorer in both AX and BR). A `ref→faction` cardMap can't represent that (last-write-wins collapses both copies to one faction), so each dealt copy is a **faction-tagged token** `"<ref>@<FACTION>"`: Lobby attaches `_token` to each pool card (`generateCubeDraftPacks` emits `c._token ?? c.reference`), and Draft/Results build a cardMap entry **per token** (`maps[`${r}@${faction}`] = {...maps[r], faction}`) while keeping base-ref entries for hero lookups. Tokens are opaque through draft rotation/picks/deck. Every render reads faction from the card OBJECT (never re-parses the ref), and set-code parsing uses segment `[1]` so the `@FACTION` suffix is harmless. `cardBaseRef(token)` (cardData.js) strips the tag — used by `buildDecklist` to merge copies into real refs for export. NOTE: the card ART still shows its printed faction frame (e.g. Nilam is a YZ print shown in the AX column — the faction ICON is AX, but the picture stays Yzmir; there's no AX printing to substitute).
